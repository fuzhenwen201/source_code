...
//格式化数据
static void format_to_string(char *str, int str_size, const char *log_level, const char* fmt, va_list* args);
//将临时数据输出到log的缓冲区
...
int SLOG_INIT(const char* config)
{
	...
	while(fgets(buf,1024,file) != NULL)  //read one line
	{
		...
		else if(strcmp(key, "slog_log_name") == 0)
		{
			...
			g_slog_setting.log_file = fopen(log_filename, "a+");
...
void slog_trace(const char* fmt, ...)
{
    if(SLOG_LEVEL_TRACE >= g_slog_setting.log_level)
    {
    	...
    	//格式化
		va_start(args, fmt);
		format_to_string(str, BUFSIZE, "TRACE", fmt, &args);
		va_end(args);
		//输出
		output_string_to_buffer(str);
    }
...
void slog_debug(const char* fmt, ...)
{
    if(SLOG_LEVEL_DEBUG >= g_slog_setting.log_level)
    {
		...
		//格式化
		va_start(args, fmt);
		format_to_string(str, BUFSIZE, "DEBUG", fmt, &args);
		va_end(args);
		//输出
		output_string_to_buffer(str);
    }
...
void slog_info(const char* fmt,  ...)
{
    if(SLOG_LEVEL_INFO >= g_slog_setting.log_level)
    {
		...
		//格式化
		va_start(args, fmt);
		format_to_string(str, BUFSIZE, "INFO", fmt, &args);
		va_end(args);
		//输出
		output_string_to_buffer(str);
    }
...
void slog_warn(const char* fmt,  ...)
{
    if(SLOG_LEVEL_WARN >= g_slog_setting.log_level)
    {
		...
		//格式化
		va_start(args, fmt);
		format_to_string(str, BUFSIZE, "WARN", fmt, &args);
		va_end(args);
		//输出
		output_string_to_buffer(str);
    }
...
void slog_error(const char* fmt, ...)
{
    if(SLOG_LEVEL_ERROR >= g_slog_setting.log_level)
    {
		...
		//格式化
		va_start(args, fmt);
		format_to_string(str, BUFSIZE, "ERROR", fmt, &args);
		va_end(args);
		//输出
		output_string_to_buffer(str);
    }
...
void format_time(char *buf)
{
    ...
    sprintf(buf,"%d-%02d-%02d %02d:%02d:%02d.%ld", 1900+t->tm_year, 1+t->tm_mon, \
    	t->tm_mday, t->tm_hour, t->tm_min, t->tm_sec, tv.tv_usec);

#if 0
    time_t now = time(NULL);
    struct tm* t = localtime(&now);
    sprintf(buf,"%d-%02d-%02d %02d:%02d:%02d",t->tm_year+1900, t->tm_mon+1, t->tm_mday, t->tm_hour, t->tm_min, t->tm_sec,t->tv_usec);
#endif
}
...
//格式化输入内容到buf
void format_to_string(char *str, int str_size, const char *log_level, const char* fmt, va_list* args)
{
	int len;
...
void flush_buffer_to_file()
{
	...
	pthread_mutex_unlock(&g_slog_setting.mutex);
	if(access(g_slog_setting.log_name,F_OK) == -1)
	{
		fclose(g_slog_setting.log_file);
		g_slog_setting.log_file = fopen(g_slog_setting.log_name, "a+");
	}
...
void rename_log_file()
{
	...
	g_slog_setting.log_file = fopen(old_log_name, "a+");
	if(g_slog_setting.log_file == NULL)
	{
		g_slog_setting.log_file = stdout;
