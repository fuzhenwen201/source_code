...
typedef struct _STTnodeDataPacketHeader
{
	
	int				starter	;					//  0x009F
	unsigned short 	tag;						//  packet tag
	unsigned short 	version;					//	2
	char 			requestid[REQUESTIDLEN+1];	//  16 bytes requestid
	char 			sessionid[SESSIONIDLEN+1];	//  16 bytes sessionid
    char            pad[2];          			//  reserved 2 byte
	unsigned int   	bodylen;					//  body len
	
} __attribute__((packed))  STTnodeDataPacketHeader;
...
namespace snetwork_xservice_tnode
{
    ...
    public:
	    ...
	    virtual unsigned int ConsumerData(STTnodeDataPacketHeader *pHeader, char *pBody) = 0;
	...
	public:
		...
		/***
			*@param   :  pRequestid  len: 16 bytes, pSessionid len: 
			*@return -1: publish error, -2: nBufLen more than TNODE_PACKET_MAXLEN  
		***/
		int 	PublishToMQ(const char * pRoutingKey,  char *pBuf, unsigned int nBufLen, 
					unsigned short tag ,const char *pRequestid = NULL, const char *pSessionid = NULL) ;

		// Adapt to the old version temporarily
		int 	PublishToMQ_v03(const char * pRoutingKey,  char *pBuf, unsigned int nBufLen, 
					unsigned short tag , const char *pRequestid = NULL, const char *pSessionid = NULL) ;

		char*  	AllocDataBuff(unsigned int nDataLen);
		void 	FreeDataBuff(char *pBuf);
	...
	private:
		...
		void	SetTnodeHeader(STTnodeDataPacketHeader *pHeader,unsigned int bodylen,unsigned short tag,
