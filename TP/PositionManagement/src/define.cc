...
const char* POSITION_DELETE_PULL_FORMAT = "DELETE FROM position_management WHERE groupid=%u and clientid=%u and margin=0";
...
const char* SG_SEND_ORDER_FORMAT ="INSERT INTO sg_send_order_log_%8d(reqid,groupid,clientid,assetid,direction,expiryperiod,amount,sgid,transfer,update_time)VALUE('%s',%u,%u,%d,%d,%d,%d,%d,%d,%ld);";

const char* SELECT_VALID_FORMAT ="SELECT balance,prevbalance,changed,margin,freemargin,profit,`status`,sgid FROM position_management WHERE groupid=%u AND clientid=%u for update";
const char* SELECT_VALID_FORMAT_STR ="SELECT balance,prevbalance,changed,margin,freemargin,profit,`status`,sgid FROM position_management WHERE groupid=%s AND clientid=%s for update";

//const char* PM_ORDER_ANS_FORMAT ="INSERT INTO pm_order_ans_log_%8d(reqid,sessionid,groupid,clientid,orderid,orderstatus,assetid,direction,amount,payout,opentime,openprice,closeprice,passive,sgid,teid,update_time)VALUE('%s','%s',%u,%u,%d,%d,%d,%d,%d,%ld,%ld,%d,%d,%d,%d,%d,%ld)";
const char* PM_ORDER_ANS_FORMAT ="INSERT INTO pm_order_ans_log_%8d(reqid,sessionid,groupid,clientid,orderid,orderstatus,assetid,direction,amount,payout,opentime,openprice,closeprice,passive,sgid,teid,update_time)VALUE('%s','%s',%u,%u,'%s',%d,%d,%d,%d,%ld,%ld,%d,%d,%d,%d,%d,%ld)";

const char* SG_ORDER_ANS_FORMAT ="INSERT INTO sg_order_ans_log_%8d(reqid,groupid,clientid,orderid,orderstatus,assetid,direction,amount,payout,opentime,openprice,closeprice,passive,sgid,teid,update_time)VALUE('%s',%u,%u,%d,%d,%d,%d,%d,%ld,%ld,%d,%d,%d,%d,%d,%ld)";
...
const char* POSITION_UPDATE_VALID_FORMAT_STR = "UPDATE position_management SET margin=margin+%s, freemargin=freemargin-%s WHERE groupid=%s AND clientid=%s";

const char* POSITION_UPDATE_PULL_FORMAT = "UPDATE position_management SET balance=%ld,freemargin=%d,prevbalance=%ld,changed=%ld,sgid=%d WHERE groupid=%u AND clientid=%u";

const char* PM_VALIDATE_ORDER_FORMAT ="INSERT INTO pm_validate_order_log_%8d(reqid,sessionid,groupid,clientid,accountid,boaccountid,assetid,direction,expiryperiod,amount,orderstatus,margin,freemargin,profit,sgid,teid,update_time)VALUE('%s','%s',%u,%u,%d,%d,%d,%d,%d,%d,%d,%ld,%ld,%d,%d,%d,%ld)";
...
const char* SG_CLIENT_LOGIN_FORMAT = "INSERT INTO sg_client_login_%8d(reqid,groupid,clientid,ipaddress,password,status,update_time)VALUE('%s',%u,%u,'%s','%s',%d,%ld);";
const char* SG_CLIENT_LOGOUT_FORMAT = "INSERT INTO sg_client_logout_%8d(reqid,clientid,groupid,status,update_time)VALUE('%s',%u,%u,%d,%ld);";
/*win: 
...
const char* POSITION_UPDATE_VALID_WIN_FORMAT_STR = "UPDATE position_management SET profit=profit+%s,freemargin=freemargin+%s+%s,margin=margin-%s,balance=balance+%s WHERE groupid=%s AND clientid=%s";
...
const char* POSITION_UPDATE_VALID_LOSS_FORMAT_STR = "UPDATE position_management SET profit=profit-%s,margin=margin-%s,balance=balance-%s WHERE groupid=%s AND clientid=%s";
...
const char* POSITION_UPDATE_VALID_REJECT_FORMAT_STR = "UPDATE position_management SET freemargin=freemargin+%s,margin=margin-%s WHERE groupid=%s AND clientid=%s";
 ...
 * Balance=Open+change+profit=Margin+FreeMargin
 */
const char* POSITION_UPDATE_VALID_CLOSE_FORMAT = "UPDATE position_management SET profit=profit+%ld,freemargin=freemargin+%ld+%d,margin=margin-%d,balance=balance+%ld WHERE groupid=%d AND clientid=%d";
const char* POSITION_UPDATE_VALID_CLOSE_FORMAT_STR = "UPDATE position_management SET profit=profit+%s,freemargin=freemargin+%s+%s,margin=margin-%s,balance=balance+%s WHERE groupid=%s AND clientid=%s";

const char* INSERT_POSITION_LOG_FORMAT ="INSERT INTO position_management_log_%8d(reqid,sessionid,groupid,clientid,sgid,accountid,balance,prevbalance,changed,margin,freemargin,profit,`type`,closebalance,`status`,`check`,update_time) VALUES('%s','%s',%u,%u,%u,%lu,%ld,%ld,%ld,%ld,%ld,%ld,%d,%ld,%d,%d,%ld)";

const char* INSERT_POSITION_INIT_FORMAT ="INSERT INTO position_management(groupid,clientid,sgid,accountid,balance,prevbalance,changed,margin,freemargin,profit,`type`,closebalance,`status`,`check`,update_time) VALUES(%d,%d,%d,%ld,%ld,%ld,%ld,%ld,%ld,%ld,%d,%ld,%d,%d,%ld)";
...
OrderAns::OrderAns() :
	...
	OrderID = (char*) malloc(SESSION_ID_LEN);
	memset(OrderID, 0, SESSION_ID_LEN);
}
...
OrderAns::OrderAns(const OrderAns& r) {
	...
	OrderID = (char*) malloc(SESSION_ID_LEN);
	memcpy(OrderID, r.OrderID, SESSION_ID_LEN);
...
const char* OrderAns::ToJson(void) {
	...
	if (Json == nullptr) {
		...
		orderid[32] = 0;
#endif
		asprintf(&Json, "{\"reqid\":\"%s\",\"sessionid\":\"%s\",\"groupid\":%d,\"clientid\":%d,\"orderid\":\"%s\",\"orderstatus\":%d,\"assetid\":%d,\"direction\":%d,\"amount\":%d,\"payout\":%ld,\"opentime\":%ld,\"openprice\":%d,\"closeprice\":%d,\"sgid\":%d,\"teid\":%d}", 
				ReqID, SessionID, GroupID, ClientID, OrderID, OrderStatus, AssetID, Direction, Amount,PayOut, OpenTime, OpenPrice, ClosePrice, SGID, TEID);
	}
...
ValidateOrder::ValidateOrder(const PositionAccount& r) {
	BOAccountID = 0;
	Direction = 0;
	OrderStatus = 0;
	Status = r.Status;
...
#endif
	if (Json == nullptr) {
		asprintf(&Json, "{\"reqid\":\"%s\",\"sessionid\":\"%s\",\"groupid\":%d,\"clientid\":%d,\"accountid\":%ld,\"boaccountid\":%d,\"assetid\":%d,\"direction\":%d,\"expiryperiod\":%d,\"amount\":%d,\"sgid\":%d,\"orderstatus\":%d,\"margin\":%ld,\"freemargin\":%ld,\"profit\":%ld,\"status\":%d,\"prevbalance\":%ld,\"changed\":%ld,\"closebalance\":%ld,\"pmid\":%s}", 
				ReqID, SessionID, GroupID, ClientID, AccountID, BOAccountID, AssetID, Direction, ExpiryPeriod, Amount, SGID, OrderStatus, Margin, FreeMargin, Profit, Status, PrevBalance, Changed, CloseBalance, g_pmid.c_str());
	}
...
#endif
		if (Status >= PULL_MSG_SIZE ||
			Status < 0 ) {
			ErrMsgIndex = PULL_MSG_SIZE - 1;
		} else {
			ErrMsgIndex = Status;
		}

		asprintf(&Json, "{\"reqid\":\"%s\",\"sessionid\":\"%s\",\"groupid\":%d,\"clientid\":%d,\"sgid\":%d,\"pmid\":%s,\"accountid\":%ld,\"prevbalance\":%ld,\"changed\":%ld,\"margin\":%ld,\"freemargin\":%ld,\"profit\":%ld,\"type\":%d,\"closebalance\":%ld,\"status\":%d,\"errmsg\":\"%s\"}", 
				 ReqID, 
				 SessionID, 
				 GroupID,
				 ClientID,
...
const char* OCPosition::ToJson(void) {
	if (Json == nullptr) {
		asprintf(&Json, "{\"reqid\":\"%s\",\"sessionid\":\"%s\",\"groupid\":%d,\"clientid\":%d,\"accountid\":%ld,\"balance\":%ld,\"prevbalance\":%ld,\"changed\":%ld,\"margin\":%ld,\"freemargin\":%ld,\"profit\":%ld,\"type\":%d,\"closebalance\":%ld,\"status\":%d,\"pmid\":%s}", 
				 ReqID, 
				 SessionID,
				 GroupID,
				 ClientID,
