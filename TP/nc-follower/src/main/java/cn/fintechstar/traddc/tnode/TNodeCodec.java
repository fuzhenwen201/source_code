package cn.fintechstar.traddc.tnode;

import cn.fintechstar.traddc.util.BytesUtil;
...
public class TNodeCodec {
	...
	public static byte[] TNodeEncode(TNode tNode){
		byte[] body = tNode.getBody();
		...
		byte[] requestId = tNode.getRequestId().getBytes();
		byte[] sessionId = tNode.getSessionId().getBytes();
		byte[] bodyLen = BytesUtil.intToBytesBig(tNode.getBodyLength());
	...
	public static TNode TNodeDecode(byte[] message){
