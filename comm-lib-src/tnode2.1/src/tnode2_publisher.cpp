...
{  
	if(argc!=6)
	{
		printf("Usage:./tnode2_publisher ip exchange routingkey content count\n");
		return -1;
	}
	char ip[32]={0};
	...
	printf("./tnode2_publisher mq_exchange_group:%s, proutingkey:%s, content:%s,count:%d\n",
		ip,argv[2],proutingkey,content,count);
