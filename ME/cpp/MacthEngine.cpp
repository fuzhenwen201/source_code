...
bool CMacthEngine::AddAskOrBidOrder(STSubmitOrderInfo* pOrderInfo, STOrderBookInfo* pOrderBookInfo)
{
	...
	while (pListOrder->size() > 0)
	{
		...
		if ((pOrderInfo->lNumber < pMarketOrder->lNumber) || (pOrderInfo->lNumber == pMarketOrder->lNumber))
		{
			//构造成交单，并写入成交明细中
			stExecOrder.lExchID = ++m_ulTransID;
			stExecOrder.lPrice = pOrderInfo->lPrice;
			stExecOrder.lNumber = pOrderInfo->lNumber;
			stExecOrder.uiSymbolID = pOrderInfo->uiExchObjID;
			if (pOrderInfo->ucTransType == EN_ORDER_BID)
			{
				stExecOrder.lBidOrderID = pOrderInfo->lSubmitOrderID;
				stExecOrder.uiBidUsrID = pOrderInfo->uiPlatformID;
				memcpy(stExecOrder.chBidUsrOrderID, pOrderInfo->chUsrOrderID, MAX_USR_ORDERID_LEN);
				stExecOrder.lAskOrderID = pMarketOrder->lSubmitOrderID;
				stExecOrder.uiAskUsrID = pMarketOrder->uiPlatformID;
				memcpy(stExecOrder.chAskUsrOrderID, pMarketOrder->chUsrOrderID, MAX_USR_ORDERID_LEN);
			}
			else
			{
				stExecOrder.lBidOrderID = pMarketOrder->lSubmitOrderID;
				stExecOrder.uiBidUsrID = pMarketOrder->uiPlatformID;
				memcpy(stExecOrder.chBidUsrOrderID, pMarketOrder->chUsrOrderID, MAX_USR_ORDERID_LEN);
				stExecOrder.lAskOrderID = pOrderInfo->lSubmitOrderID;
				stExecOrder.uiAskUsrID = pOrderInfo->uiPlatformID;
				memcpy(stExecOrder.chAskUsrOrderID, pOrderInfo->chUsrOrderID, MAX_USR_ORDERID_LEN);
			}
		...
		//	stExecOrder.lBidOrderID = (pOrderInfo->ucTransType == EN_ORDER_BID) ? pOrderInfo->lSubmitOrderID : pMarketOrder->lSubmitOrderID;
			...
			if (pOrderInfo->lNumber == pMarketOrder->lNumber)
			{
				//弹出元素，回收内存
			...
			{
				pMarketOrder->lNumber -= pOrderInfo->lNumber;
		...
		else
		{
			...
			stExecOrder.lExchID = ++m_ulTransID;
			stExecOrder.lPrice = pOrderInfo->lPrice;
			stExecOrder.lNumber = pMarketOrder->lNumber;
			stExecOrder.uiSymbolID = pOrderInfo->uiExchObjID;
...
//			stExecOrder.lBidOrderID = (pOrderInfo->ucTransType == EN_ORDER_BID) ? pOrderInfo->lSubmitOrderID : pMarketOrder->lSubmitOrderID;
			...
			pOrderInfo->lNumber -= pMarketOrder->lNumber;
			...
			while (1)
			{
				...
				if (m_pOutputRingBuf->getFreeSize() >= sizeof(ST_MARKET_HEADER_INFO) + iDataLen)
				{
					...
					for (; re_ask_iter != iterOrderBook->second.mapAskOrderBook.rend(); re_ask_iter++)
					{
						...
						stSigOrder.lNumber = re_ask_iter->second.lTotalNumber;
			...
			while (1)
			{
				...
				if (m_pOutputRingBuf->getFreeSize() >= sizeof(ST_MARKET_HEADER_INFO) + iDataLen)
				{
					...
					for (; re_bid_iter != iterOrderBook->second.mapBidOrderBook.rend(); re_bid_iter++)
					{
						...
						stSigOrder.lNumber = re_bid_iter->second.lTotalNumber;
						m_pOutputRingBuf->pushData(&stSigOrder, sizeof(STSigOrderBookItem));
					}
					break;
				}
	...
	while (iter != (*pOrderBook).end())
	{
		...
		while (iterTimeOrder != (iter->second).mulmapOrder.end())
		{
				...
				iterTimeOrder->second->ucTransStatus == EN_TRANS_STATUS_CPL_TRANS)
			{
				...
				(iter->second).lTotalNumber -= iterTimeOrder->second->lNumber;
			...
			memcpy(stExecOrder.chAskUsrOrderID, iterTimeOrder->second->chUsrOrderID, MAX_USR_ORDERID_LEN);
			stExecOrder.uiAskUsrID = iterTimeOrder->second->uiPlatformID;

			stExecOrder.lBidOrderID = pSubmitOrder->lSubmitOrderID;
			memcpy(stExecOrder.chBidUsrOrderID, pSubmitOrder->chUsrOrderID, MAX_USR_ORDERID_LEN);
			stExecOrder.uiBidUsrID = pSubmitOrder->uiPlatformID;
			stExecOrder.uiSymbolID = pSubmitOrder->uiExchObjID;
			stExecOrder.ucExchType = 1;
			//如果市场化订单小于委托单的数量，则在相应的委托单上修改剩余数量并且修改成交标记位
			//然后发送成交订单，发送完后则直接返回
			if (pSubmitOrder->lNumber < iterTimeOrder->second->lNumber)
			{
				stExecOrder.lNumber = pSubmitOrder->lNumber;
				(iter->second).lTotalNumber -= pSubmitOrder->lNumber;
				iterTimeOrder->second->lNumber -= pSubmitOrder->lNumber;
			...
			else
			{
				...
				{
					pSubmitOrder->lNumber -= iterTimeOrder->second->lNumber;
					pSubmitOrder->ucTransStatus = EN_TRANS_STATUS_PART_TRANS;
				}
				stExecOrder.lNumber = iterTimeOrder->second->lNumber;
				WriteTransferOrder(&stExecOrder);
				//回收该委托订单
				UINT uiIndex = EffectiveMod(iterTimeOrder->second->uiOrderIndex, MAX_ORDER_INDEX);
				if (0 == m_pOrderCache[uiIndex].lSubmitOrderID || m_pOrderCache[uiIndex].lSubmitOrderID == iterTimeOrder->second->lSubmitOrderID)
...
// 					m_pOrderCache[uiIndex].pOrderAddr = NULL;
			...
			stExecOrder.uiAskUsrID = pSubmitOrder->uiPlatformID;
			memcpy(stExecOrder.chAskUsrOrderID, pSubmitOrder->chUsrOrderID, MAX_USR_ORDERID_LEN);
			stExecOrder.lBidOrderID = iterTimeOrder->second->lSubmitOrderID;
			stExecOrder.uiBidUsrID = iterTimeOrder->second->uiPlatformID;
			memcpy(stExecOrder.chBidUsrOrderID, iterTimeOrder->second->chUsrOrderID, MAX_USR_ORDERID_LEN);
			stExecOrder.uiSymbolID = pSubmitOrder->uiExchObjID;
			stExecOrder.ucExchType = 1;
			//如果市场化订单小于委托单的数量，则在相应的委托单上修改剩余数量并且修改成交标记位
			//然后发送成交订单，发送完后则直接返回
...
bool CMacthEngine::CancelSubmitOrder(const STSubmitOrderInfo* pOrderInfo, STOrderBookInfo* pOrderBookInfo)
{
	...
	else if (m_pOrderCache[uiIndex].lSubmitOrderID == pOrderInfo->lSubmitOrderID)
	{
		if (m_pOrderCache[uiIndex].pOrderAddr > 0)
		{
			...
			stExecOrder.lNumber = pPreOrder->lNumber;
			stExecOrder.uiSymbolID = pPreOrder->uiExchObjID;
			BYTE ucTransStatus = pPreOrder->ucTransStatus;
			//modified by lsq 0319
			//在这里和savin讨论过，在撤单里没有部分撤单的概念，所以这里取消判断
...
// 				pPreOrder->ucTransStatus = EN_TRANS_STATUS_PART_CANCEL;
			...
			if (pPreOrder->ucTransType == EN_ORDER_BID)
			{
				...
				memcpy(stExecOrder.chBidUsrOrderID, pPreOrder->chUsrOrderID, MAX_USR_ORDERID_LEN);
				stExecOrder.uiBidUsrID = pPreOrder->uiPlatformID;
			}
			else
			{
				stExecOrder.lAskOrderID = pPreOrder->lSubmitOrderID;
				memcpy(stExecOrder.chAskUsrOrderID, pPreOrder->chUsrOrderID, MAX_USR_ORDERID_LEN);
				stExecOrder.uiAskUsrID = pPreOrder->uiPlatformID;
			}
		...
		if (iter != (*pMapOrderBook).end())
		{
			...
			while (range.first != range.second)
			{
				...
				if (range.first->second->lSubmitOrderID == pOrderInfo->lSubmitOrderID)
				{
					...
					{
						(iter->second).lTotalNumber -= range.first->second->lNumber;
					...
					if (bIsNeedCancel)
					{
						...
						stExecOrder.lNumber = range.first->second->lNumber;
						stExecOrder.uiSymbolID = range.first->second->uiExchObjID;
						if (range.first->second->ucTransType == EN_ORDER_BID)
						{
							stExecOrder.lBidOrderID = range.first->second->lSubmitOrderID;
							memcpy(stExecOrder.chBidUsrOrderID, range.first->second->chUsrOrderID, MAX_USR_ORDERID_LEN);
							stExecOrder.uiBidUsrID = range.first->second->uiPlatformID;
						}
						else
						{
							stExecOrder.lAskOrderID = range.first->second->lSubmitOrderID;
							memcpy(stExecOrder.chAskUsrOrderID, range.first->second->chUsrOrderID, MAX_USR_ORDERID_LEN);
							stExecOrder.uiAskUsrID = range.first->second->uiPlatformID;
						}
						stExecOrder.ucExchType = 0;
						WriteTransferOrder(&stExecOrder);
					}
	...
	while (rev_iter_bid != stOrderBook.mapBidOrderBook.rend())
	{
		...
		while (iter_time_bid != (rev_iter_bid->second).mulmapOrder.end())
		{
			...
			{     
				...
				memcpy(stExecOrder.chAskUsrOrderID, iter_time_ask->second->chUsrOrderID, MAX_USR_ORDERID_LEN);
				stExecOrder.uiAskUsrID = iter_time_ask->second->uiPlatformID;
				stExecOrder.lBidOrderID = iter_time_bid->second->lSubmitOrderID;
				memcpy(stExecOrder.chBidUsrOrderID, iter_time_bid->second->chUsrOrderID, MAX_USR_ORDERID_LEN);
				stExecOrder.uiBidUsrID = iter_time_bid->second->uiPlatformID;
				stExecOrder.uiSymbolID = iter_time_bid->second->uiExchObjID;
				stExecOrder.ucExchType = 1;
				//如果买单数量小于等于卖单，则将该买单全部成交
 				if ((iter_time_bid->second->lNumber < iter_time_ask->second->lNumber) || (iter_time_bid->second->lNumber == iter_time_ask->second->lNumber))
				{
					stExecOrder.lNumber = iter_time_bid->second->lNumber;
					WriteTransferOrder(&stExecOrder);
					//修改买卖单的总数信息
					(rev_iter_bid->second).lTotalNumber -= iter_time_bid->second->lNumber;
	     			(iter_ask->second).lTotalNumber -= iter_time_bid->second->lNumber;
...
//					iter_time_ask->second->fNumber -= iter_time_bid->second->fNumber;
					...
					if (iter_time_bid->second->lNumber == iter_time_ask->second->lNumber)
					{
						iter_time_ask->second->lNumber -= iter_time_bid->second->lNumber;
				...
				{
					stExecOrder.lNumber = iter_time_ask->second->lNumber;
					WriteTransferOrder(&stExecOrder);
					//修改买卖单的总数信息
					(rev_iter_bid->second).lTotalNumber -= iter_time_ask->second->lNumber;
					(iter_ask->second).lTotalNumber -= iter_time_ask->second->lNumber;

					iter_time_bid->second->ucTransStatus = EN_TRANS_STATUS_PART_TRANS;
					iter_time_bid->second->lNumber -= iter_time_ask->second->lNumber;
					//清除卖单的相关信息
					iter_time_ask->second->ucTransStatus = EN_TRANS_STATUS_CPL_TRANS;
