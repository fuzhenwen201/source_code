...
public:
  ...
  void fromString( const std::string& str )
  {
    ...
    {
      m_targetCompID = str.substr(second+2, fourth - second - 2);
      m_sessionQualifier = "";
      m_clientIp = str.substr(fourth+2);
    }else if(first != third && second == fourth){
      ...
      m_targetCompID = str.substr(second+2, third - second - 2);
      m_sessionQualifier = str.substr(third+1, fourth - third - 1);
      m_clientIp = str.substr(fourth+2);
    }
    toString(m_frozenString);
  }
  ...
  SessionID operator~() const
  {
    return SessionID( m_beginString, SenderCompID( m_targetCompID ),
                      TargetCompID( m_senderCompID ), m_sessionQualifier, m_clientIp );
  }
