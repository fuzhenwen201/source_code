...
namespace FIX44
{
    ...
    class OrderBooks : public Message
    {
    public:
        OrderBooks() : Message(MsgType()) {}
        OrderBooks(const FIX::Message& m) : Message(m) {}
        OrderBooks(const Message& m) : Message(m) {}
        OrderBooks(const OrderBooks& m) : Message(m) {}
        static FIX::MsgType MsgType() { return FIX::MsgType("G1007"); }
