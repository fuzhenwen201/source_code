...
{  
 ...
 virtual void onMessage( const Heartbeat&, const FIX::SessionID& ) 
  ...
  virtual void onMessage( const ConfirmationRequest&, const FIX::SessionID& ) 
    { throw FIX::UnsupportedMessageType(); }
  virtual void onMessage( const HistQuotReq&, const FIX::SessionID& )
  { throw FIX::UnsupportedMessageType(); }
  virtual void onMessage( const OrdRetransReq&, const FIX::SessionID& )
  { throw FIX::UnsupportedMessageType(); }
  virtual void onMessage( HistQuotReq&, const FIX::SessionID& ) {}
  virtual void onMessage( OrdRetransReq&, const FIX::SessionID& ) {}
...
public:
              ...
              const FIX::SessionID& sessionID )
  {
    ...
    if(msgTypeValue == "G1001")
      onMessage( (const HistQuotReq&)message, sessionID );
    else
    if(msgTypeValue == "G1003")
      onMessage( (const OrdRetransReq&)message, sessionID );
...
void crack( Message& message, 
            const FIX::SessionID& sessionID )
  {
    ...
    if(msgTypeValue == "G1001")
      onMessage( (HistQuotReq&)message, sessionID );
    else
    if(msgTypeValue == "G1003")
      onMessage( (OrdRetransReq&)message, sessionID );
    else onMessage( message, sessionID );
  }
