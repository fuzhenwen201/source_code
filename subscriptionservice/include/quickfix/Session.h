...
public:
  ...
  const MessageStoreFactory& getMessageStoreFactory() const
  { return m_messageStoreFactory; }

  const TimeRange& getTimeRange() const
  { return m_sessionTime; }

  const LogFactory* getLogFactory() const
  { return m_pLogFactory; }

  const int& getheartBtInt() const
  { return m_heartBtInt; }
  ...
  void next( const std::string&, const UtcTimeStamp& timeStamp, bool queued = false, int sock = 0 );
